...
function GetLastFak(sTipe: string): string; overload;
function GetLastFak(sTipe: string; tgl: TDateTime): string; overload;
...
function GetSatuan(kode_brg: String; var satuan: string): String;

function GetHPP(sKodeBrg: string; sGudang: string): Real; overload;
function GetHPP(sKodeBrg: string): Real; overload;
function GetUnitKTG(sKodeBrg: string): String;

function GenerateKode(sHeader: string): string;
function GetConvSatuan(sKodeBrg:string; satuan: string): real;
function GetLastKodeBrg(sHeader: string): string;

function AkunExists(NoAkun: string): Boolean;
function GetNamaAkun(NoAkun: string): String;
function GetDefaultAkun(jenis: string; dk: string): String; overload;
function GetDefaultAkun(jenis: string): string; overload;
function GetDefaultAkunBrg(sKode: string; sJenis: string): String;
function GetDefaultAkunSupp(sKode: string): String;

implementation

function GetDefaultAkun(jenis: string; dk: string): String;
var
  q: TZQuery;
begin
  q := OpenRS('SELECT akun FROM tbl_default_akun WHERE jenis = ''%s'' AND LEFT(dk,1) = ''%s''',
    [jenis, dk]);
  Result := q.FieldByName('akun').AsString;
  q.Close;
end;

function GetDefaultAkun(jenis: string): String;
var
  q: TZQuery;
begin
  q := OpenRS('SELECT noakun FROM tbl_akundefault WHERE jenis = ''%s''',[jenis]);
  Result := q.FieldByName('noakun').AsString;
  q.Close;
end;
...
begin
  if sTipe = 'brg_in' then
    ...
    depan := 'SPP/'
  else if sTipe = 'quotetion' then
    depan := 'QUO/'
  else if sTipe = 'kpo' then
    depan := 'KPO/'
  else if sTipe = 'inquiry-sheet' then
    depan := 'IS/'
  else if sTipe = 'lembur' then
    depan := 'SPL/'
  else if sTipe = 'jurnal-kerja' then
    depan := 'RND/'

  // SPK EXTRUSI DKK
  else if sTipe = 'spk-mixing' then
    depan := 'SPK/MIX/'
  else if sTipe = 'spk-extrusi' then
    depan := 'SPK/EXT/'
  else if sTipe = 'spk-printing' then
    depan := 'SPK/PRN/'
  else if sTipe = 'spk-guset' then
    depan := 'SPK/GST/'
  else if sTipe = 'spk-converting' then
    depan := 'SPK/CON/'
  else if sTipe = 'spk-packing' then
    depan := 'SPK/PCK/'
  else if sTipe = 'spk-sorting' then
    depan := 'SPK/SRT/'
  else if sTipe = 'spk-crusher' then
    depan := 'SPK/CRS/'
  else if sTipe = 'spk-washing' then
    depan := 'SPK/WSH/'
  else if sTipe = 'spk-pelet' then
    depan := 'SPK/PLT/'
  else if sTipe = 'spk-injection' then
    depan := 'SPK/INJ/'
  else if sTipe = 'spk-casting' then
    depan := 'SPK/CST/'
  else if sTipe = 'spk-manual' then
    depan := 'SPK/MAN/'
  else if sTipe = 'draft-cp' then
    depan := 'DRF/'
  else if sTipe = 'ambil-bahan' then
    depan := 'ABB/'
  else if sTipe = 'retur-bahan' then
    depan := 'RBB/'
  else if sTipe = 'costcalculation' then
    depan := 'CC/'

  else if sTipe = 'komplain-customer' then
    depan := 'KCS/'

  else if sTipe = 'improvement' then
    depan := 'IMP/'
  ...
  // ACC
    ...
    depan := 'PEJ/'
  else if sTipe = 'invoice' then
    depan := 'SI/'
  else if sTipe = 'permintaan_pembelian' then 
    depan := 'PR/'
  else if sTipe = 'kas_keluar' then
    depan := 'KK/'
    else if sTipe = 'kas_masuk' then
    depan := 'KM/'
            else if sTipe = 'bank_keluar' then
    depan := 'BK/'
    else if sTipe = 'bank_masuk' then
    depan := 'BM/'
    else if sTipe = 'uang_muka' then
    depan := 'UM/'
...
function GetLastFak(sTipe: string; Tgl: TDateTime): string;

  function InGetFaktur(Tanggal: TDateTime): string;
  const
...
var
  q: TZQuery;
begin
  q := OpenRS('SELECT PASSWORD(''%s'') AS pass', [sPassword]);
...
begin
  ...
  q := OpenRS('SELECT * FROM tbl_head_no WHERE header = ''' + head + '''');
...
function GetHPP(sKodeBrg: string; sGudang: string): Real;
var
  q: TZQuery;
begin
  {
  q := OpenRS('SELECT IFNULL(hpp,0) hpp FROM tbl_history WHERE id = (SELECT MAX(id) FROM tbl_history WHERE kode_brg = ''%s'')',
    [sKodeBrg]);

  if q.IsEmpty then begin
    q := OpenRS('SELECT IFNULL(hpp,0) hpp FROM tbl_barang WHERE kode = ''%s''',[sKodeBrg]);
    if not q.IsEmpty then
  ...
  q := OpenRS('SELECT sf_get_hpp(''%s'',''%s'') hpp',[sKodeBrg, sGudang]);
  Result := q.FieldByName('hpp').AsFloat;
  q.Close;
  
end;

function GetHPP(sKodeBrg: string): Real;
var
  q: TZQuery;
begin
  q := OpenRS('SELECT sf_get_hpp(''%s'') hpp',[sKodeBrg]);
  Result := q.FieldByName('hpp').AsFloat;
  q.Close;
end;
...
var
  q: TZQuery;
begin
  q := OpenRS('SELECT unit_ktg FROM tbl_barang WHERE kode = ''%s''',[sKodeBrg]);
  if not q.IsEmpty then
    Result := q.FieldByName('unit_ktg').AsString
  else
    Result := '';
end;
...
var
  ...
  b: Boolean;
begin
  q := OpenRS('SELECT unitid, conv1, conv2, conv2, conv4, conv5, sat1, sat2, sat3, sat4, sat5 ' +
    'FROM tbl_barang WHERE kode = ''%s''',[sKodeBrg]);
    
  b := false;
  if not q.IsEmpty then begin
    if q.FieldByName('unitid').AsString = satuan then begin
      b := True;
      Result := 1;
    end
    else begin
      for i := 0 to q.Fields.Count - 1 do begin
        if Copy(q.Fields[i].DisplayName,1,3) = 'sat' then begin
          if q.Fields[i].Value = satuan then begin
            Result := q.FieldByName('conv' + Copy(q.Fields[i].DisplayName, Length(q.Fields[i].DisplayName),1)).AsFloat;
            b := True;
            Break;
          end;
        end;
...
begin
  ...
  if q.IsEmpty then
    ...
    head := head + Copy('000',0,3 - Length(IntToStr(counter))) + IntToStr(counter);
  end;
  Result := head;
end;
...
var
  q: TZQuery;
begin
  q := OpenRS('SELECT noakun FROM tbl_coa WHERE noakun = ''%s''', [NoAkun]);
  if q.IsEmpty then
    Result := False
  else
    Result := True;
...
var
  q: TZQuery;
begin
  q := OpenRS('SELECT nama FROM tbl_coa WHERE noakun = ''%s''',[NoAkun]);
  if q.IsEmpty then
    Result := ''
  else
    Result := q.FieldByname('nama').AsString;
...
var
  ...
  q: TZQuery;
begin
  sSQL := 'SELECT akun_' + sJenis + ' FROM tbl_barang WHERE kode = ''' + sKode + '''';
  q := OpenRS(sSQL);
  if not q.IsEmpty then
    Result := q.FieldByName('akun_' + sJenis).AsString
  else
    Result := '';
  q.Close;
end;
...
var
  q : TZQuery;
begin
  q := OpenRS('SELECT akun_hutang FROM tbl_supplier WHERE kode = ''%s''',[sKode]);
  if not q.IsEmpty then
    Result := q.FieldByName('akun_hutang').AsString
  else
    Result := '';
  q.Close;
end;
...
var
  q: TZQuery;
begin
  q := OpenRS('SELECT unitid FROM tbl_barang WHERE kode = ''%s''',[kode_brg]);
  if not q.IsEmpty then begin
    Result := q.FieldByName('unitid').AsString;
    satuan := q.FieldByName('unitid').AsString;
  end
  else
    Result := '';
  q.Close;
