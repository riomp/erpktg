  ...
  if cxsTotalQty.Value = 0 then begin
    ...
    try
      ...
      for i := 0 to nxGrd.RowCount - 1 do begin
        ...
        in_det.FieldByName('pajak').AsString := nxGrd.Cell[nxColPPN.Index, i].AsString;
        in_det.Post;
...
var
  q, q_poh, qBrg, qsj, qb, qso_det, qpb: TZQuery;
  i: Integer;
begin
...
var
  ...
  sNoPB, sNoJ, AD, AK, sAkunBrg, sAkun: string;
  q, qso, qcek, qSO_V_SJ, q_sh, q_sd, q_sa, qjh, qjd, qj, qSupp: TZQuery;
          ...
          if not q_sd.IsEmpty then begin
            ...
            while not q_sd.Eof do begin
              ...
              brg_det.Close;
               ...
               qSupp := OpenRS('SELECT * FROM tbl_supplier WHERE kode = ''%s''',[cxLUSupp.Editvalue]);
               brg := OpenRS('SELECT * FROM tbl_barang WHERE kode = ''%s''',
                [q_sd.FieldByName('kode_brg').AsString]);
              ...
              qjd.FieldByName('no_trans').AsString := sNoPB;
...
begin
  ...
  if not q.IsEmpty then begin
    ...
    while not q.Eof do begin
      ...
      nxGrd.Cell[nxColPPN.Index, i].AsString := q.FieldByName('pajak').AsString;
