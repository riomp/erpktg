...
uses
  ...
  IdSSL, IdSSLOpenSSL, IdText, IdAttachmentFile, cxRadioGroup,
  ZAbstractDataset;

type
  TfrmSO_MTS = class(TfrmTplTrans)
    cxLabel1: TcxLabel;
    ...
    cxrWIPAwal: TcxRadioButton;
    cxrWIPAkhir: TcxRadioButton;
    ...
    procedure cxChkWIPClick(Sender: TObject);
    procedure cxLUBrgPropertiesChange(Sender: TObject);
    procedure FormShow(Sender: TObject);
  private
    mUserDept,mNoMTS: string;
  ...
  public
    ...
    property NoMTS: string read mNoMTS write mNoMTS;
  end;

var
  frmSO_MTS: TFrmSO_MTS;
...
{$R *.dfm}
  ...
  procedure TFrmSO_MTS.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(Params);
  Params.ExStyle := Params.ExStyle or WS_EX_APPWINDOW;
end;

procedure TFrmSO_MTS.FormCreate(Sender: TObject);
var
  q: TZQuery;
  lst: TStringList;
  i: Integer;
...
begin
  ...
  cxrWIPAwal.Enabled := False;
  cxrWIPAkhir.Enabled := False;

  zSatuan.Open;
end;

procedure TFrmSO_MTS.btnBaruClick(Sender: TObject);
begin
  inherited;
...
function TFrmSO_MTS.CekKodeBrg(sKodeBrg : String; Row: Integer) : Boolean;
var
  i: integer;
  tmp: Boolean;
begin
...
procedure TFrmSO_MTS.NxEdit1DblClick(Sender: TObject);
var
  f: TFrmCari;
  s: TStringList;
  fKode: TField;
...
procedure TFrmSO_MTS.nxGrdAfterEdit(Sender: TObject; ACol, ARow: Integer;
  Value: WideString);
var
  q: TZQuery;
  i: integer;
...
procedure TFrmSO_MTS.HitungTotal;
var
  i: integer;
  tmp, tmpQty: Real;
begin
...
procedure TFrmSO_MTS.btnBatalClick(Sender: TObject);
begin
  inherited;
  ClearAll;
  Self.Jenis := '';
...
procedure TFrmSO_MTS.btnSimpanClick(Sender: TObject);
var
  ...
  //end
    ...
    try
      ...
      if cxChkWIP.Checked then begin
        in_head.FieldByName('f_wip').AsInteger := 1;
        if cxrWIPAwal.Checked then
          in_head.FieldByName('f_wip_awal').AsInteger := 1;
        if cxrWIPAkhir.Checked then
          in_head.FieldByName('f_wip_akhir').AsInteger := 1;
      end;
...
procedure TFrmSO_MTS.nxGrdCellDblClick(Sender: TObject; ACol, ARow: Integer);
var
  q: TZQuery;
begin
...
procedure TFrmSO_MTS.btnHapusClick(Sender: TObject);
var
  q: TZQuery;
  i: Integer;
begin
...
procedure TFrmSO_MTS.btnTambahClick(Sender: TObject);
var
  s: TStringList;
  fKode: TField;
  ACol, ARow, i: Integer;
...
procedure TFrmSO_MTS.cxdTglTrsPropertiesChange(Sender: TObject);
begin
  {
  ...
  if Self.Jenis = 'tambah' then
   cxtNoTrans.Text := GetLastFak('mts',cxdTglTrs.Date);
end;

procedure TFrmSO_MTS.btnEdit2Click(Sender: TObject);
var
  q, qBrg: TZQuery;
  i: Integer;
begin
  ...
  if q.FieldByName('f_wip').AsInteger = 1 then
    cxChkWIP.Checked := True
  else
    cxChkWIP.Checked := False;

  if q.FieldByName('f_wip_awal').AsInteger = 1 then
    cxrWIPAwal.Checked := True;
  if q.FieldByName('f_wip_akhir').AsInteger = 1 then
    cxrWIPAkhir.Checked := True;
...
procedure TFrmSO_MTS.cxCmbKategoriSOPropertiesChange(Sender: TObject);
var
  sKat: string;
  i: Integer;
  tx: TcxCheckListBoxItem;
...
procedure TFrmSO_MTS.ResetCheckEmail;
var
  lst: TStringList;
  tx: TcxCheckListBoxItem;
  i: Integer;
...
procedure TFrmSO_MTS.SendEmailSO(sNoBukti: string);
var
  em: ThreadKirimEmail;
...
function TFrmSO_MTS.GenerateHTML(sNoBukti: string) : string;
var
  lst: TStringList;
  sMemo: string;
  i: Integer;
...
procedure TFrmSO_MTS.cxLUCustPropertiesChange(Sender: TObject);
begin
  try
    if Self.Jenis <> 'edit' then begin
...
procedure TFrmSO_MTS.NxButtonEdit1ButtonClick(Sender: TObject);
var
  i: integer;
begin
  i := nxGrd.SelectedRow;
...
procedure TFrmSO_MTS.cxLUBrgKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TFrmSO_MTS.cxsQtyKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then begin
    btnTambahClick(nil);
    cxsQty.EditValue := 0;
...
procedure TFrmSO_MTS.GetDaftarEmail;
var
  lst: TStringList;
  cmb : TcxComboBox;
  i,j: Integer;
...
procedure TFrmSO_MTS.Button1Click(Sender: TObject);
begin
  inherited;
  MsgBox(IntToStr(cxCmbKategoriSO.ItemIndex));
end;

procedure TFrmSO_MTS.btnCariCustClick(Sender: TObject);
var
  f: TfrmCari;
  s: TStringList;
  fKode: TField;
...
procedure TFrmSO_MTS.pnlMemoExit(Sender: TObject);
begin
  pnlMemo.Visible := False;
end;

procedure TFrmSO_MTS.NxTextColumn4ButtonClick(Sender: TObject);
begin
  cxMemo.Lines.Text := nxGrd.Cell[CKET,nxGrd.SelectedRow].AsString;
  pnlMemo.Visible := True;
  pnlMemo.SetFocus;
...
procedure TFrmSO_MTS.btnSimpanKetClick(Sender: TObject);
begin
...
procedure TFrmSO_MTS.btnBatalMemoClick(Sender: TObject);
begin
  inherited;
  pnlMemo.Visible := False;
end;

procedure TFrmSO_MTS.cxMemoExit(Sender: TObject);
begin
  inherited;
  btnSimpanKetClick(nil);
end;

procedure TFrmSO_MTS.idSMTPWorkBegin(ASender: TObject;
...
procedure TFrmSO_MTS.idSMTPWorkEnd(ASender: TObject;
  AWorkMode: TWorkMode);
begin
  try
    dm.zConn.StartTransaction;
...
procedure TFrmSO_MTS.btnCariBrgClick(Sender: TObject);
var
  f: TFrmCari;
  s: TStringList;
  fKode: TField;
...
procedure TfrmSO_MTS.cxChkWIPClick(Sender: TObject);
begin
  inherited;
  if cxChkWIP.Checked then begin
    cxrWIPAwal.Enabled := True;
    cxrWIPAkhir.Enabled := True;
  end
  else begin
    cxrWIPAwal.Enabled := False;
    cxrWIPAkhir.Enabled := False;
    cxrWIPAwal.Checked := False;
    cxrWIPAkhir.Checked := False;
  end;
end;

procedure TfrmSO_MTS.cxLUBrgPropertiesChange(Sender: TObject);
var
  sSatuan : string;
begin
  inherited;
...
procedure TfrmSO_MTS.FormShow(Sender: TObject);
var
  q, qBrg: TZQuery;
  i: Integer;
begin
  ...
  if Self.Jenis = 'edit' then begin
      ...
      q := OpenRS('SELECT a.*,b.nama FROM tbl_so_head a left join tbl_customer b ' +
          'on a.kode_customer = b.kode WHERE a.no_bukti = ''%s''', [NoMTS]);
