...
uses
  ...
  cxButtons, ExtCtrls,ShellAPI, IniFiles, dxGDIPlusClasses;
...
begin
  ...
  if tbl.Locate('nama;password',
    ...
    Aplikasi.UserDept := tbl.FieldByName('user_dept').AsString;
    ...
    Aplikasi.EmailUser := q.FieldByname('alamat_email').AsString;
    Aplikasi.EmailPassword := q.FieldByName('password_email').AsString;
    ...
    Ini.WriteString('database','username',sUser);
    Ini.WriteString('database','dept',Aplikasi.UserDept);
...
begin
  if Trim(cxtNamaUser.Text) = '' then begin
    ...
    if not DoLogin then begin
      ...
      //frmUtama.imgBg.Width := frmUtama.Width;
      //frmUtama.imgBg.Height := frmUtama.Height;
    end;
  end;
end;
