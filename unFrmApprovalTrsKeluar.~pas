...
type
  TfrmApprovalTrsKeluar = class(TForm)
    nxGrd: TNextGrid;
...
var
  frmApprovalTrsKeluar: TfrmApprovalTrsKeluar;
...
procedure TfrmApprovalTrsKeluar.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(Params);
  Params.ExStyle := Params.ExStyle or WS_EX_APPWINDOW;
end;

procedure TfrmApprovalTrsKeluar.Button1Click(Sender: TObject);
begin
  Close;
end;

procedure TfrmApprovalTrsKeluar.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Release;
end;

procedure TfrmApprovalTrsKeluar.nxGrdCellClick(Sender: TObject; ACol,
  ARow: Integer);
begin
  zqrDetKor.Close;
...
procedure TfrmApprovalTrsKeluar.FormCreate(Sender: TObject);
begin
  if aplikasi.UserDept = 'WHL' then begin
    ...
    if aplikasi.NamaUser = 'ANTON' then
      zqrKorHead.SQL.Text := 'SELECT a.no_bukti, a.tanggal, a.user, a.user_dept ' +
        'FROM tbl_trskeluar_head a WHERE a.f_approval = 0 AND a.user_dept = ''WHL'' ' +
        'AND a.user = ''ANTON'' ' + 
        'ORDER BY a.tanggal DESC';
...
procedure TfrmApprovalTrsKeluar.btnApprovalClick(Sender: TObject);
var
  btn: TButton;
  sJenis: string;
  i : Integer;
        ...
        if nxGrd.Cell[nxColApp.Index,i].AsBoolean then begin
          ...
          while not q.Eof do begin
            ...
            dm.zConn.ExecuteDirect('UPDATE tbl_barang SET stok = stok - ' +
              StringReplace(q.FieldByName('qty').AsString,',','.',[rfReplaceAll]) +
            ...
            hist.FieldByName('user').AsString := qh.FieldByName('user').AsString;
            hist.FieldByName('user_dept').AsString := qh.FieldByName('user_dept').AsString;
...
procedure TfrmApprovalTrsKeluar.SendEmail(lstEmail, lstNoBukti: TStringList; sJenis: string);
var
  email : TIdMessage;
  lst: TStringList;
  bdy: TIdText;
...
procedure TfrmApprovalTrsKeluar.idSMTPWorkEnd(ASender: TObject;
  AWorkMode: TWorkMode);
begin
