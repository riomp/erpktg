...
procedure TfrmProsesRK.FormShow(Sender: TObject);
var
  q, qImg, q2: TZQuery;
  i: Integer;
  sStatus: string;
...
procedure TfrmProsesRK.FormCreate(Sender: TObject);
begin
  //self.Refresh;
  //scrollBox.Refresh;
...
procedure TfrmProsesRK.btnSimpanClick(Sender: TObject);
var
  tbl: TZTable;
...
procedure TfrmProsesRK.btnBatalClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmProsesRK.btnLihatFotoClick(Sender: TObject);
var
  f: TfrmImageViewerRK;
begin
...
procedure TfrmProsesRK.btnSOPenggantiClick(Sender: TObject);
var
  f: TfrmCariSO;
  q: TZQuery;
  noindex_s: String;
...
procedure TFrmProsesRK.SendEmail(lstEmail: TStringList);
begin
  //
end;

procedure TfrmProsesRK.chkBrgDigantiClick(Sender: TObject);
begin
  if not chkBrgDiganti.Checked then
    cxtNoSOGanti.Text := '';
end;

procedure TfrmProsesRK.chkNotAppClick(Sender: TObject);
begin
  if chkNotApp.Checked then
    chkAppBDDir.Checked := False;
end;

procedure TfrmProsesRK.chkAppBDDirClick(Sender: TObject);
begin
  if chkAppBDDir.Checked then
    chkNotApp.Checked := False;
end;

procedure TfrmProsesRK.UpdateTglSelesai(sNoRK: string);
var
  sql: string;
begin
  try
