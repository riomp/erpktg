object frmPilihPO: TfrmPilihPO
  ...
  object zqrPO: TZReadOnlyQuery
    Connection = dm.zConn
    SQL.Strings = (
      'SELECT a.tanggal,a. no_bukti '
      'FROM tbl_po_head a'
      'LEFT JOIN tbl_po_det b ON a.no_bukti = b.no_bukti'
      
        'WHERE a.kode_supp = :id_supplier AND b.kode_brg = :id_brg AND a.' +
        'f_completed = 0 AND a.f_approval = 1'
      'ORDER BY a.tanggal ')
    Params = <
      item
        DataType = ftUnknown
  ...
  object zqrGdg: TZReadOnlyQuery
    Connection = dm.zConn
    SQL.Strings = (
      'SELECT kode, nama FROM tbl_gudang')
    Params = <>
    Left = 480
    Top = 112
  end
