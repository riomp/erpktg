...
type
  TfrmMstBrg_KTG = class(TfrmTplMaster)
    ...
    cxlCP: TcxLookupComboBox;
    btnSimpanAlt: TButton;
    Label75: TLabel;
    Label76: TLabel;
    cxtInline: TcxTextEdit;
    Label77: TLabel;
    ...
    cxlAkunJenisBarang: TcxLookupComboBox;
    zqrAkunJB: TZQuery;
    dsAkunJB: TDataSource;
    zqrCoa: TZQuery;
    dsCoa: TDataSource;
    Label82: TLabel;
    zqrAfval: TZQuery;
    dsAfval: TDataSource;
    cxlAfval: TcxLookupComboBox;
    cxColBomLayer4: TcxGridColumn;
    cxColBomLayer5: TcxGridColumn;
    cxColBomLayer6: TcxGridColumn;
    cxColBomLayer7: TcxGridColumn;
    cxColBomLayer8: TcxGridColumn;
    cxLabel64: TcxLabel;
    ...
    procedure cxTblaLTDataControllerRecordChanged(
      ADataController: TcxCustomDataController; ARecordIndex,
      AItemIndex: Integer);
    procedure cxlCPPropertiesEditValueChanged(Sender: TObject);
    procedure btnSimpanAltClick(Sender: TObject);
    procedure cxlAkun1PropertiesChange(Sender: TObject);
    procedure cxlAkunJenisBarangPropertiesChange(Sender: TObject);
    procedure cxlAkun8PropertiesChange(Sender: TObject);
    procedure cxlAkun2PropertiesChange(Sender: TObject);
    procedure cxlAkun3PropertiesChange(Sender: TObject);
    procedure cxlAkun4PropertiesChange(Sender: TObject);
    procedure cxlAkun5PropertiesChange(Sender: TObject);
    procedure cxlAkun6PropertiesChange(Sender: TObject);
    procedure cxlAkun7PropertiesChange(Sender: TObject);
  private
...
begin
  ...
  else if ((cxlKelompok.EditValue = 'C') AND (cxtSubGroup.Text='') ) or ((cxlKelompok.EditValue = 'D')AND (cxtSubGroup.Text=''))  then begin
          MsgBox('Sub Group Harus di isi.');
          cxtSubGroup.SetFocus;
  end
  else begin
  ...
  if f then begin
    with q do begin
      ...
      if not VarIsNull(cxlCP.EditValue) then
      FieldByName('no_cp').AsString           := cxlCP.EditValue ;
      ...
      FieldByName('inline').AsString          := cxtInline.Text ;
      ...
      if cxlMesin.EditValue<>null then
      FieldByName('kode_mesin').asString      := cxlMesin.EditValue ;
      if cxsSpeed.Value<>0 then
      FieldByName('speed').asFloat            := cxsSpeed.Value;

       FieldByName('diameter').asFloat            := cxsdiameter.Value;
        FieldByName('jumlah_plong').asFloat            := cxsjmlPlong.Value;
      ...
      if cxlAfval.EditValue<>null then
      FieldByName('kode_afval').AsString  := cxlAfval.EditValue;
      FieldByName('jenis').AsString := cxCmbJenis.Text;
      ...
      FieldByName('proses_gusset').AsString := cxtG_Proses.Text ;
      ...
      FieldByName('mt_layer1').Value := cxs_MS_Layer1.Value;
      FieldByName('mt_layer2').Value := cxs_MS_Layer2.Value;
      FieldByName('mt_layer3').Value  := cxs_MS_Layer3.Value;
      FieldByName('mt_layer4').Value  := cxs_MS_Layer4.Value;
      FieldByName('mt_layer5').Value  := cxs_MS_Layer5.Value;
      FieldByName('mt_layer6').Value  := cxs_MS_Layer6.Value;
      FieldByName('mt_layer7').Value  := cxs_MS_Layer7.Value;
      FieldByName('mt_layer8').Value  := cxs_MS_Layer8.Value;

      FieldByName('mtp_layer1').Value := cxs_MS_PLayer1.Value;       
      FieldByName('mtp_layer2').Value := cxs_MS_PLayer2.Value;
      FieldByName('mtp_layer3').Value := cxs_MS_PLayer3.Value;
      FieldByName('mtp_layer4').Value := cxs_MS_PLayer4.Value;
      FieldByName('mtp_layer5').Value := cxs_MS_PLayer5.Value;
      FieldByName('mtp_layer6').Value := cxs_MS_PLayer6.Value;
      FieldByName('mtp_layer7').Value := cxs_MS_PLayer7.Value;
      FieldByName('mtp_layer8').Value := cxs_MS_PLayer8.Value;
      ...
      FieldByName('printdet_colours_inout').AsFloat := cxsPDColoursIO.Value;
        ...
        // default akun
        if not VarIsNull(cxlAkunJenisBarang.EditValue) then
        FieldByName('nama_tpl_akun').AsString         := cxlAkunJenisBarang.EditValue;
        if not VarIsNull(cxlAkun1.EditValue) then
        FieldByName('akun_persediaan').AsString       := cxlAkun1.EditValue;
        if not VarIsNull(cxlAkun2.EditValue) then
        FieldByName('akun_penjualan').AsString        := cxlAkun2.EditValue;
        if not VarIsNull(cxlAkun3.EditValue) then
        FieldByName('akun_retur_penjualan').AsString  := cxlAkun3.EditValue;
        if not VarIsNull(cxlAkun4.EditValue) then
        FieldByName('akun_diskon_penjualan').AsString := cxlAkun4.EditValue;
        if not VarIsNull(cxlAkun5.EditValue) then
        FieldByName('akun_barang_terkirim').AsString  := cxlAkun5.EditValue;
        if not VarIsNull(cxlAkun6.EditValue) then
        FieldByName('akun_hpp').AsString              := cxlAkun6.EditValue;
        if not VarIsNull(cxlAkun7.EditValue) then
        FieldByName('akun_retur_pembelian').AsString  := cxlAkun7.EditValue;
        if not VarIsNull(cxlAkun8.EditValue) then
        FieldByName('akun_blm_tertagih').AsString     := cxlAkun8.EditValue;
  ...
  for i := 0 to ComponentCount - 1 do begin
    if (Components[i].Tag = 11) then begin
      if (Components[i].ClassName = 'TcxTextEdit') then
        (Components[i] as TcxTextEdit).Text := '';
      if (Components[i].ClassName = 'TcxLookupComboBox') then
        (Components[i] as TcxLookupComboBox).Text := '';
    end;
  end;
...
begin
  ...
  with q do begin
    ...
    CxlCP.EditText            := FieldByName('no_cp').AsString ;
    ...
    cxttpj.Text               := FieldByName('t_panjangjadi').asString;
    ...
    cxsDiameter.Value    := FieldByName('diameter').asFloat;
    cxsjmlPlong.Value    := FieldByName('jumlah_plong').asFloat;
    ...
    cxtG_Proses.Text            := FieldByName('proses_gusset').AsString;
    ...
    cxs_MS_Layer1.Value := FieldByName('mt_layer1').Value;
    cxs_MS_Layer2.Value := FieldByName('mt_layer2').Value;
    cxs_MS_Layer3.Value  := FieldByName('mt_layer3').Value;
    cxs_MS_Layer4.Value  := FieldByName('mt_layer4').Value;
    cxs_MS_Layer5.Value  := FieldByName('mt_layer5').Value;
    cxs_MS_Layer6.Value  := FieldByName('mt_layer6').Value;
    cxs_MS_Layer7.Value  := FieldByName('mt_layer7').Value;
    cxs_MS_Layer8.Value  := FieldByName('mt_layer8').Value;

    cxs_MS_PLayer1.Value := FieldByName('mtp_layer1').Value;
    cxs_MS_PLayer2.Value := FieldByName('mtp_layer2').Value;
    cxs_MS_PLayer3.Value := FieldByName('mtp_layer3').Value;
    cxs_MS_PLayer4.Value := FieldByName('mtp_layer4').Value;
    cxs_MS_PLayer5.Value := FieldByName('mtp_layer5').Value;
    cxs_MS_PLayer6.Value := FieldByName('mtp_layer6').Value;
    cxs_MS_PLayer7.Value := FieldByName('mtp_layer7').Value;
    cxs_MS_PLayer8.Value := FieldByName('mtp_layer8').Value;

    cxtKodeFina.Text := FieldByName('fina_kode').AsString;
    cxtNamaFina.Text := FieldByName('fina_nama').AsString;

    // kategori dan subkategori
    cxCmbKategori.Text := FieldByName('kategori').AsString;
    cxCmbSubKategori.Text := FieldByName('subkategori').AsString;
    cxCmbJenis.Text := FieldByName('jenis').AsString;

    // default akun
    cxlAkunJenisBarang.EditValue := FieldByName('nama_tpl_akun').AsString;
    cxlAkun1.EditValue := FieldByName('akun_persediaan').AsString;
    cxlAkun2.EditValue := FieldByName('akun_penjualan').AsString;
    cxlAkun3.EditValue := FieldByName('akun_retur_penjualan').AsString;
    cxlAkun4.EditValue := FieldByName('akun_diskon_penjualan').AsString;
    cxlAkun5.EditValue := FieldByName('akun_barang_terkirim').AsString;
    cxlAkun6.EditValue := FieldByName('akun_hpp').AsString;
    cxlAkun7.EditValue := FieldByName('akun_retur_pembelian').AsString;
    cxlAkun8.EditValue := FieldByName('akun_blm_tertagih').AsString;
    cxtAkun1.EditValue := FieldByName('akun_persediaan').AsString;
    cxtAkun2.EditValue := FieldByName('akun_penjualan').AsString;
    cxtAkun3.EditValue := FieldByName('akun_retur_penjualan').AsString;
    cxtAkun4.EditValue := FieldByName('akun_diskon_penjualan').AsString;
    cxtAkun5.EditValue := FieldByName('akun_barang_terkirim').AsString;
    cxtAkun6.EditValue := FieldByName('akun_hpp').AsString;
    cxtAkun7.EditValue := FieldByName('akun_retur_pembelian').AsString;
    cxtAkun8.EditValue := FieldByName('akun_blm_tertagih').AsString;
  ...
  if Copy(cxtKode.Text,1,1) = 'A' then begin
   ...
   if Copy(cxtKode.Text,1,3) = 'B01' then begin
    cxColBomBahan.Visible := True ;
      ...
      cxColBomLayer4.Visible := True ;
      cxColBomLayer5.Visible := True ;
      cxColBomLayer6.Visible := True ;
      cxColBomLayer7.Visible := True ;
      cxColBomLayer8.Visible := True ;
      end
      else begin
      ...
      cxColBomLayer4.Visible := False ;
      cxColBomLayer5.Visible := False ;
      cxColBomLayer6.Visible := False ;
      cxColBomLayer7.Visible := False ;
      cxColBomLayer8.Visible := False ;
   end;
  ...
  if Copy(cxtKode.Text,1,3) = 'B01' then begin
    z := OpenRS('SELECT a.*, b.deskripsi,c.* FROM tbl_formula_subdet a ' +
      ...
      'WHERE a.nama="A" and c.nama_formula = "A" and a.kode = ''%s''',[cxtKode.text]);
    end
  else begin
       z := OpenRS('SELECT a.*, b.deskripsi,c.* FROM tbl_formula_subdet a ' +
    'LEFT JOIN tbl_barang b ON a.kode_brg = b.kode ' +
    'LEFT JOIN tbl_mat_saran c ON a.kode_brg = c.kode_brg and a.kode=c.kode ' +
    'WHERE a.nama="A" and a.kode = ''%s''',[cxtKode.text]);
    end;
  while not z.Eof do begin
    with cxTblBOM.DataController do begin
      ...
      Values[i, cxColBomLayer4.Index] := z.FieldByName('layer4').Value;
      Values[i, cxColBomLayer5.Index] := z.FieldByName('layer5').Value;
      Values[i, cxColBomLayer6.Index] := z.FieldByName('layer6').Value;
      Values[i, cxColBomLayer7.Index] := z.FieldByName('layer7').Value;
      Values[i, cxColBomLayer8.Index] := z.FieldByName('layer8').Value;
...
begin
  ...
  cxCmbKategori.Properties.Items.Clear;
  while not q.Eof do begin
  ...
  // isi combo subkategori
  q := OpenRS('SELECT * FROM tbl_subkategori_brg ORDER BY subkategori');
  cxCmbSubKategori.Properties.Items.Clear;
  while not q.Eof do begin
    cxCmbSubKategori.Properties.Items.Add(q.FieldByName('subkategori').AsString);
    q.Next;
  end;
  q.Close;

  // jenis barang
  q := OpenRS('SELECT DISTINCT jenis FROM tbl_barang ORDER BY jenis');
  cxCmbJenis.Properties.Items.Clear;
  while not q.Eof do begin
    cxCmbJenis.Properties.Items.Add(q.FieldByName('jenis').AsString);
    q.Next;
  end;
  q.Close;
...
begin
  ...
  if (cxlKelompok.EditValue = 'A') or (cxlKelompok.EditValue = 'B') then begin
    ...
    cxlAfval.Enabled := True;
  end
  else begin
    cxlCustomer.Enabled := false;
    cxlSupplier.Enabled := true;
    cxtSubGroup.Enabled := True;
    cxlAfval.Enabled := False;
  end;

  if (cxlKelompok.EditValue = 'A') then
      cxCmbJenis.Text := 'PERSEDIAAN BARANG JADI'
  else if (cxlKelompok.EditValue = 'B') then
      cxCmbJenis.Text := 'PERSEDIAAN BARANG DALAM PROSES'
  else if  (cxlKelompok.EditValue = 'C') then
      cxCmbJenis.Text := 'PERSEDIAAN BAHAN BAKU'
  else if  (cxlKelompok.EditValue = 'D') then
      cxCmbJenis.Text := 'PERSEDIAAN BAHAN PEMBANTU'
   else if  (cxlKelompok.EditValue = 'E') then
      cxCmbJenis.Text := 'PERSEDIAAN SPARE PART'
   else if  (cxlKelompok.EditValue = 'F') then
      cxCmbJenis.Text := 'PERSEDIAAN AFVALAN DAN PELET' ;
end;
...
begin
  ...
  dm.zConn.ExecuteDirect(
    Format('DELETE FROM tbl_formula_subdet WHERE kode = ''%s'' and nama="A" ',[cxtKode.text]));
 ...
 if AItemIndex = cxColBOmDesc.Index then begin
    ...
    if ADataController.Values[ARecordIndex, cxColJnsBom.Index] = 'VOLUME' then
      ADataController.Values[ARecordIndex, cxColPersen.Index] := 0;
        ...
        (AItemIndex = cxColBomLayer3.Index) or
        (AItemIndex = cxColBomLayer4.Index) or
        (AItemIndex = cxColBomLayer5.Index) or
        (AItemIndex = cxColBomLayer6.Index) or
        (AItemIndex = cxColBomLayer7.Index) or
        (AItemIndex = cxColBomLayer8.Index) then begin
            if Copy(cxtKode.Text,1,3) = 'B01' then begin
             try
            with ADataController do begin
              a := VarToStr(Values[ARecordIndex, cxColBomLayer1.Index]) ;
              
              Values[ARecordIndex, cxColPersen.Index] :=
                ((Values[ARecordIndex, cxColBomLayer1.Index] / 100) * cxs_MS_PLayer1.Value) +
                ((Values[ARecordIndex, cxColBomLayer2.Index] / 100) * cxs_MS_PLayer2.Value) +
                ((Values[ARecordIndex, cxColBomLayer3.Index] / 100) * cxs_MS_PLayer3.Value) +
                ((Values[ARecordIndex, cxColBomLayer4.Index] / 100) * cxs_MS_PLayer4.Value) +
                ((Values[ARecordIndex, cxColBomLayer5.Index] / 100) * cxs_MS_PLayer5.Value) +
                ((Values[ARecordIndex, cxColBomLayer6.Index] / 100) * cxs_MS_PLayer6.Value) +
                ((Values[ARecordIndex, cxColBomLayer7.Index] / 100) * cxs_MS_PLayer7.Value) +
                ((Values[ARecordIndex, cxColBomLayer8.Index] / 100) * cxs_MS_PLayer8.Value);
         ...
         if Copy(ADataController.Values[ARecordIndex, cxColBomKode.Index],1,1) = 'B' then begin
           r := OpenRS('select a.*,b.conversi from tbl_unitset_det as a join (SELECT unitsetid,`convert` as conversi FROM tbl_unitset_det where unitid=''%s'') as b on a.unitsetid=b.unitsetid WHERE a.unitsetid=''%s'' and a.unitid="KG" order by no asc',[cxlSatuanDet.EditValue,cxlSatuan.EditValue]);
           ADataController.Values[ARecordIndex, cxColBomQty.Index] :=  (ADataController.Values[ARecordIndex, cxColPersen.Index] / 100) * ((1000/r.FieldByName('conversi').AsFloat)*r.FieldByName('convert').AsFloat);
      end
      else begin
...
begin
  ...
  if pg.ActivePageIndex = 2 then begin
    ...
    //konversi
      cxTblConversi.DataController.RecordCount := 0;
        r := OpenRS('select a.*,b.conversi from tbl_unitset_det as a join (SELECT unitsetid,`convert` as conversi FROM tbl_unitset_det where unitid=''%s'') as b on a.unitsetid=b.unitsetid WHERE a.unitsetid=''%s'' order by no asc',[cxlSatuanDet.EditValue,cxlSatuan.EditValue]);
      while not r.Eof do begin
        with cxTblConversi.DataController do begin
          ...
          Values[j, cxColKonversi.Index] :=((1/r.FieldByName('conversi').AsFloat)*r.FieldByName('convert').AsFloat);
        end;
        r.Next;
      end;
      r.Close;
    ...
    if Copy(cxtKode.Text,1,3) = 'B01' then begin
      ...
      q := OpenRS('SELECT * FROM tbl_formula_subdet WHERE nama=''A'' and kode = ''%s''',[cxtKode.Text]);
...
begin
  ...
  if (AItem.Index = cxColPersen.Index) or (AItem.Index = cxColBomLayer1.Index) or
    ...
    (AItem.Index = cxColBomLayer4.Index) or (AItem.Index = cxColBomLayer5.Index) or
    (AItem.Index = cxColBomLayer6.Index) or (AItem.Index = cxColBomLayer7.Index) or
    (AItem.Index = cxColBomLayer8.Index) or
...
begin
  ...
  if AItemIndex = cxCol_MS_Deskripsi.Index then begin
    ...
    (AItemIndex = cxCol_MS_Layer3.Index)
    then begin
    try
      with ADataController do begin
        Values[ARecordIndex, cxCol_MS_Mono.Index] :=
          ((Values[ARecordIndex, cxCol_MS_Layer1.Index] / 100) * cxs_MS_PLayer1.Value) +
          ((Values[ARecordIndex, cxCol_MS_Layer2.Index] / 100) * cxs_MS_PLayer2.Value) +
          ((Values[ARecordIndex, cxCol_MS_Layer3.Index] / 100) * cxs_MS_PLayer3.Value) ;
      end;
    except
    end;
  end;
...
begin
  if Copy(cxtKode.Text,1,3) = 'B01' then begin
      ...
      with cxTblBOM.DataController do begin
        ...
        dm.zConn.ExecuteDirect('DELETE FROM tbl_mat_saran WHERE kode = ''' + cxtKode.Text + ''' and nama_formula="A"');
        ...
        for i := 0 to RecordCount - 1 do begin
          ...
          q.FieldByName('layer4').Value := Values[i, cxColBomLayer4.Index];
          q.FieldByName('layer5').Value := Values[i, cxColBomLayer5.Index];
          q.FieldByName('layer6').Value := Values[i, cxColBomLayer6.Index];
          q.FieldByName('layer7').Value := Values[i, cxColBomLayer7.Index];
          q.FieldByName('layer8').Value := Values[i, cxColBomLayer8.Index];
          q.FieldByName('mono').Value := Values[i, cxColPersen.Index];
          q.FieldByName('nama_formula').AsString := 'A';

          q.FieldByName('mtp_layer1').Value := cxs_MS_PLayer1.Value;
          q.FieldByName('mtp_layer2').Value := cxs_MS_PLayer2.Value;
          q.FieldByName('mtp_layer3').Value := cxs_MS_PLayer3.Value;
          q.FieldByName('mtp_layer4').Value := cxs_MS_PLayer4.Value;
          q.FieldByName('mtp_layer5').Value := cxs_MS_PLayer5.Value;
          q.FieldByName('mtp_layer6').Value := cxs_MS_PLayer6.Value;
          q.FieldByName('mtp_layer7').Value := cxs_MS_PLayer7.Value;
          q.FieldByName('mtp_layer8').Value := cxs_MS_PLayer8.Value;
          q.Post;
        end;
        q.Close;
...
begin
  try
    if (cxsDiameter.EditValue<>0) and (cxsjmlPlong.EditValue<>0) then begin
      persen_plong:= ((3.14/4)*(cxsDiameter.EditValue*cxsDiameter.EditValue)* cxsjmlPlong.EditValue)/(cxsLebarBuka.Value * cxsPanjang.Value);
      Result := (cxsBeratKotor.Value - (persen_plong * cxsBeratKotor.Value));
    end
    else begin
...
begin
  try
    cxs_MS_Total.Value := cxs_MS_Layer1.Value + cxs_MS_Layer2.Value + cxs_MS_Layer3.Value + cxs_MS_Layer4.Value + cxs_MS_Layer5.Value +
    cxs_MS_Layer6.Value + cxs_MS_Layer7.Value + cxs_MS_Layer8.Value ;
...
begin
  inherited;
   with ADataController do begin
    ...
    Values[ARecordIndex, cxColBomLayer4.Index] := 0;
    Values[ARecordIndex, cxColBomLayer5.Index] := 0;
    Values[ARecordIndex, cxColBomLayer6.Index] := 0;
    Values[ARecordIndex, cxColBomLayer7.Index] := 0;
    Values[ARecordIndex, cxColBomLayer8.Index] := 0;
...
procedure TfrmMstBrg_KTG.cxlCPPropertiesEditValueChanged(Sender: TObject);
var
  q : TZQuery ;
begin
  inherited;
  try
    q := OpenRS('SELECT * FROM tbl_cp WHERE no_cp= ''%s''',[cxlCP.Text ]);
    if not q.IsEmpty then begin
          cxlCustomer.EditValue := q.FieldByName('kode_customer').AsString;
          ...
          cxsTebal.Value:= q.FieldByName('tebal').AsFloat ;
          cxsBeratKotor.Value:= q.FieldByName('berat_kotor').AsFloat  ;
          cxsBeratBersih.Value:= q.FieldByName('berat_bersih').AsFloat;
          cxsBeratMeter.Value:= q.FieldByName('berat_meter').AsFloat ;
          cxsBeratJadi.Value:= q.FieldByName('berat_fg').AsFloat;
          cxsLebarBuka.Value:= q.FieldByName('lebar_buka').AsFloat ;
          cxsLebarJadi.value:= q.FieldByName('lebar_jadi').AsFloat;
          cxsPanjang.Value:= q.FieldByName('panjang').AsFloat;
          cxsPanjangJadi.Value:= q.FieldByName('panjang_jadi').AsFloat;
          cxsLebarTube.Value:= q.FieldByName('lebar_tube').AsFloat;
          cxsPanjangPlong.Value:= q.FieldByName('panjang_plong').AsFloat;
          cxsLebarPlong.Value:= q.FieldByName('lebar_plong').asFloat;
          cxsTopFlod.Value:= q.FieldByName('top_fold').AsFloat;
          cxsGusset.Value:= q.FieldByName('gusset').AsFloat;
          cxsLebarHandle.Value:= q.FieldByName('lebar_handle').AsFloat ;
          ...
          cxlIS.EditValue := q.FieldByName('no_is').Value;
      end
      else begin
          cxlCustomer.EditValue := q.FieldByName('kode_customer').AsString;
          cxtFungsi.Text := q.FieldByName('fungsi').AsString;
...
procedure TfrmMstBrg_KTG.btnSimpanAltClick(Sender: TObject);
var
  z : TZQuery ;
  i : Integer ;
begin
...
procedure TfrmMstBrg_KTG.cxlAkun1PropertiesChange(Sender: TObject);
var
  s: string;
  t: TcxTextEdit;
begin
...
procedure TfrmMstBrg_KTG.cxlAkunJenisBarangPropertiesChange(
  Sender: TObject);
var
  q: TZQuery;
  i: Integer;
...
begin
  inherited;
  sNama := cxlAkunJenisBarang.EditValue;
...
procedure TfrmMstBrg_KTG.cxlAkun8PropertiesChange(Sender: TObject);
begin
  inherited;
  cxtAkun8.Text := zqrCoa.FieldByName('noakun').AsString;
end;

procedure TfrmMstBrg_KTG.cxlAkun2PropertiesChange(Sender: TObject);
begin
  inherited;
 cxtAkun2.Text := zqrCoa.FieldByName('noakun').AsString;
end;

procedure TfrmMstBrg_KTG.cxlAkun3PropertiesChange(Sender: TObject);
begin
  inherited;
 cxtAkun3.Text := zqrCoa.FieldByName('noakun').AsString;
end;

procedure TfrmMstBrg_KTG.cxlAkun4PropertiesChange(Sender: TObject);
begin
  inherited;
   cxtAkun4.Text := zqrCoa.FieldByName('noakun').AsString;
end;

procedure TfrmMstBrg_KTG.cxlAkun5PropertiesChange(Sender: TObject);
begin
  inherited;
   cxtAkun5.Text := zqrCoa.FieldByName('noakun').AsString;
end;

procedure TfrmMstBrg_KTG.cxlAkun6PropertiesChange(Sender: TObject);
begin
  inherited;
   cxtAkun6.Text := zqrCoa.FieldByName('noakun').AsString;
end;

procedure TfrmMstBrg_KTG.cxlAkun7PropertiesChange(Sender: TObject);
begin
  inherited;
   cxtAkun7.Text := zqrCoa.FieldByName('noakun').AsString;
end;
