    ...
    try
      ...
      for i := 0 to cxTbl.DataController.RowCount - 1 do begin
        ...
        if VarIsNull(cxTbl.DataController.Values[i, cxColProject.Index]) = True then begin
          ...
          in_det.FieldByName('project').AsString := inttostr(cxTbl.DataController.Values[i, cxColProject.Index]);
        end;
    ...
    while not q.Eof do begin
      with cxTbl.DataController do begin
        ...
        if Values[i, cxColTax.Index] = 'PPN' then begin
          ...
          t1 := t * 2/100;
          Values[i, cxColSubtotal.Index] := t + t1;
        end else begin
...
begin
  ...
  q := OpenRS('SELECT a.*, b.deskripsi, b.unitid FROM tbl_fobj_det a LEFT JOIN tbl_barang b ON a.kode_brg = b.kode WHERE no_bukti = ''%s''',[cxlNoPP.EditValue]);
  if not q.IsEmpty then begin
    with cxTbl.DataController do begin
      BeginUpdate;
      try
