...
var
  f: TfrmProsesMS;
  fSPKAssembly: TFrmTrsSPKAssembly_NEW;
  q: TZQuery;
...
begin
  try
    ...
    if q.FieldByName('f_completed').AsInteger = 1 then begin
      ...
      //recIdx := tblView2.Controller.SelectedRecords[i].Index;
      //s := tblView2.DataController.Values[recIdx,1];
      //MsgBox(s);

      //MsgBox(zqrSPK.FieldByName('no_mo').AsString);

      if sJenisSPK = 'BARANG JADI' then begin
        f := TfrmProsesMS.Create(Self);
        f.NoMO := zqrSPK.FieldByName('no_mo').AsString;
        f.NoSPK := sNoSPK;
        f.Jenis := 'edit';
        f.JenisSPK := zqrSPK.FieldByName('jenis_spk').AsString;
        f.ShowModal;
      end
      else if sJenisSPK = 'MIXING / COMPOUND' then begin
        f := TfrmProsesMS.Create(Self);
        f.NoMO := zqrSPK.FieldByName('no_mo').AsString;
        f.NoSPK := sNoSPK;
        f.Jenis := 'edit';
      ...
      else if sJenisSPK = 'PELETAN' then begin
        f := TfrmProsesMS.Create(Self);
        f.NoMO := zqrSPK.FieldByName('no_mo').AsString;
        f.NoSPK := sNoSPK;
        f.Jenis := 'edit';
      ...
      else if sJenisSPK = 'SPK ASSEMBLY' then begin
        fSPKAssembly := TFrmTrsSPKAssembly_NEW.Create(Self);
        fSPKAssembly.Jenis := 'edit';
        fSPKAssembly.KodeBrg := zqrSPK.FieldByName('kode_brg').AsString;
        fSPKAssembly.NoSO := zqrSPK.FieldByName('no_so').AsString;
        fSPKAssembly.NoMO := zqrSPK.FieldBYName('no_mo').AsString;
        fSPKAssembly.NoSPK := sNoSPK;
        fSPKAssembly.ShowModal;
      end;
