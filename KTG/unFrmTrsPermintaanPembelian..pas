...
begin
  ...
  cxtNoTrans.Text := GetLastFak('permintaan_pembelian');
...
function TFrmTrsPermintaanPembelian.CekKodeBrg(sKodeBrg : String; Row: Integer) : Boolean;
var
  i: integer;
  tmp: Boolean;
begin
...
procedure TFrmTrsPermintaanPembelian.NxEdit1DblClick(Sender: TObject);
var
  f: TFrmCari;
  s: TStringList;
  fKode: TField;
...
procedure TFrmTrsPermintaanPembelian.nxGrdAfterEdit(Sender: TObject; ACol, ARow: Integer;
  Value: WideString);
var
  q: TZQuery;
  i: integer;
...
procedure TFrmTrsPermintaanPembelian.HitungTotal;
var
  i: integer;
  tmp, tmpQty: Real;
begin
...
procedure TFrmTrsPermintaanPembelian.nxGrdCellDblClick(Sender: TObject; ACol, ARow: Integer);
var
  fs: TFrmCariSo;
begin
...
procedure TFrmTrsPermintaanPembelian.btnTambahClick(Sender: TObject);
var
  s: TStringList;
  fKode: TField;
  ACol, ARow: Integer;
...
begin
  ...
  self.Jenis := 'edit';
  cxtNoTrans.Text := zqrNoBukti1.FieldByName('no_bukti').AsString;
  cxtNoTrans.Properties.ReadOnly := True;

  cxdTglTrs.Date := zqrNoBukti1.FieldByName('tanggal').AsDateTime;
  ...
  q2 := OpenRS('SELECT * FROM v_fobj_det WHERE no_bukti = ''%s''',
    [zqrNoBukti1.FieldByName('no_bukti').AsString]);
...
procedure TFrmTrsPermintaanPembelian.AddRow(sNoSO, sKodeBrg: string; qty_so, qty_g02: real);
var
  f: TFrmCari;
  fs: TfrmCariSO;
  s: TStringList;
...
procedure TFrmTrsPermintaanPembelian.cxLUBrgKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    btnTambahClick(nil);
end;

procedure TFrmTrsPermintaanPembelian.nxBeNoSOButtonClick(Sender: TObject);
var
  f: TfrmCari;
  ARow, ACol: Integer;
  s: TStringList;
...
procedure TFrmTrsPermintaanPembelian.nxBtnEditSOButtonClick(Sender: TObject);
var
  f: TfrmCari;
  ARow, ACol: Integer;
  s: TStringList;
...
procedure TFrmTrsPermintaanPembelian.btnCariBrgClick(Sender: TObject);
var
  f: TFrmCari;
  s: TStringList;
  fKode: TField;
...
procedure TFrmTrsPermintaanPembelian.btnEdit3Click(Sender: TObject);
var
  q, qBrg, q2: TZQuery;
  i: Integer;
begin
