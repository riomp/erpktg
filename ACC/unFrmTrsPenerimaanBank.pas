...
uses
  ...
  ZAbstractRODataset, ZDataset, cxNavigator, cxDBNavigator, cxGridLevel,
  cxGridCustomTableView, cxGridTableView, cxGridDBTableView, cxClasses,
  cxGridCustomView, cxGrid, NxColumnClasses, NxColumns, NxScrollControl,
  NxCustomGridControl, NxCustomGrid, NxGrid, cxDropDownEdit, cxCalendar,
  cxCheckBox, cxMaskEdit, cxLookupEdit, cxDBLookupEdit, cxLabel;

type
  TfrmTrsPenerimaanBank = class(TfrmTransBaru)
    cxLabel1: TcxLabel;
  ...
  public
    ...
    property NoBM: string read mNoBM write mNoBM;
  end;

var
  frmTrsPenerimaanBank: TfrmTrsPenerimaanBank;
...
procedure TfrmTrsPenerimaanBank.ClearAll;
begin
 nxGrd.ClearRows;
  cxdTgl.Text := '';
  cxlAkun.Text := '';
...
procedure TfrmTrsPenerimaanBank.FormCreate(Sender: TObject);
var
  q: TZQuery;
begin
  inherited;
...
procedure TfrmTrsPenerimaanBank.FormShow(Sender: TObject);
var
  q : TZQuery ;
begin
  ...
  if Self.Jenis= 'edit' then begin
    ...
    'AND debet > 0',[NoBM]);
      ...
      q := OpenRS('SELECT * FROM v_jurnal ' +
        'WHERE no_jurnal = ''%s'' ' +
        'AND kredit > 0 ORDER BY urutan',[mNoBM]);
...
procedure TfrmTrsPenerimaanBank.HitungTotal;
var
  i,total: Integer;
begin
  total := 0;
...
procedure TfrmTrsPenerimaanBank.cxdTglExit(Sender: TObject);
begin
  inherited;
 if mJenisAkun = 'non-tunai' then begin
...
procedure TfrmTrsPenerimaanBank.cxdTglPropertiesChange(Sender: TObject);
begin
  inherited;
  if Self.Jenis = 'tambah' then begin
      cxtNoTrans.Text := GetLastFak('bank_masuk',cxdTgl.Date);
  end;
end;

procedure TfrmTrsPenerimaanBank.cxlAkunPropertiesChange(Sender: TObject);
begin
  inherited;
...
procedure TfrmTrsPenerimaanBank.nxGrdAfterEdit(Sender: TObject; ACol,
...
procedure TfrmTrsPenerimaanBank.nxGrdCellDblClick(Sender: TObject; ACol,
  ARow: Integer);
begin
  inherited;
...
procedure TfrmTrsPenerimaanBank.btnSimpanClick(Sender: TObject);
var
  i, id: Integer;
  f: boolean;
  qjh, qjd: TZQuery;
...
begin
  ...
  if Trim(cxlAkun.Text) = '' then begin
    ...
    try
      ...
      if Self.Jenis = 'tambah' then begin
        sNoJ := GetLastFak('bank_masuk',cxdTgl.Date);
